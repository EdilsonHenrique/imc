...
// Obter pagamentos realizados
$sqlPagamentos = "SELECT * FROM pagamentos WHERE id_venda = ?";
$stmtPagamentos = $conn->prepare($sqlPagamentos);
$stmtPagamentos->bind_param("i", $id);
$stmtPagamentos->execute();
$pagamentos = $stmtPagamentos->get_result()->fetch_all(MYSQLI_ASSOC);

$venda['pagamentos'] = $pagamentos;
$venda['valor_pago'] = getValorPago($id);
$venda['valor_pendente'] = $venda['valor_total'] - $venda['valor_pago'];
